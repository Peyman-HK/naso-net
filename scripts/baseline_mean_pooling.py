...
Usage:
    & "D:\PLOS ONE\vpi_env\Scripts\python.exe" L213_baseline_mean_pooling.py
"""
...
# ── Model: ResNet50 + Mean Pooling (NO WMV) ──────────────
def build_resnet_mean_pooling(window_length, target_size, activation="gelu", lr=1e-3):
    ...
    simple mean of per-frame sigmoid probabilities.
    """
    ...
    # Per-frame probability — same as WMV model
    p = TimeDistributed(Dense(1, activation="sigmoid"), name="frame_prob")(x)

    # Simple mean pooling instead of WMV
    # p shape: (batch, window_length, 1)
...
# ═════════════════════════════════════════════════════════
print("=" * 65)
print("  L213: ResNet50 + Mean Pooling Baseline")
...
for fold_idx, (train_idx, test_idx) in enumerate(gkf.split(labels, labels, groups)):
    ...
    checkpoint_path = str(
        OUTPUT_DIR / f"mean_pool_fold{fold_num}.weights.h5"
    )
    periodic_path = str(
        OUTPUT_DIR / f"mean_pool_fold{fold_num}_epoch{{epoch:02d}}.weights.h5"
    )
...
# ── Print results ─────────────────────────────────────────
print(f"\n{'=' * 65}")
print(f"  ResNet50 + Mean Pooling — {n_folds}-fold patient-wise CV")
print(f"{'=' * 65}")
